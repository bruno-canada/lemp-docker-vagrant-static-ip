# LEMP based on Docker and Vagrant
...
- Access the virtual machine (VM) SSH:
`vagrant ssh`

- Restart Docker daemon from inside the VM (Running Linux Alpine)
...
- Delete Docker containers, networks, volumes, and images created by Docker compose for this setup
```
docker-compose down
```

- Builds, (re)creates, starts, and attaches to containers for service in detached mode. (Run containers in the background)
```
docker-compose up -d
```
